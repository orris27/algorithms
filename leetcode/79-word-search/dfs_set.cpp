...
class Solution {
public:
    bool dfs(const vector<vector<char>>& board, string word, int i, int j, set<pair<int, int>>& path, int index) {
        ...
        // move up
        //debug("move up\n");
        if (i - 1 >= 0 && board[i-1][j] == word[index] && path.find(pair<int, int>{i-1, j}) == path.end() && dfs(board, word, i-1, j, path, index + 1))
            return true;

        // move down
        //debug("move down\n");
        if (i + 1 < board.size() && board[i+1][j] == word[index] && path.find(pair<int, int>{i+1, j}) == path.end() && dfs(board, word, i+1, j, path, index + 1))
            return true;

        // move to left
        //debug("move to left\n");
        if (j - 1 >= 0 && board[i][j-1] == word[index] && path.find(pair<int, int>{i, j-1}) == path.end() && dfs(board, word, i, j-1, path, index + 1))
            return true;
            
        // move to right
        //debug("move to right\n");
        if (j + 1 < board[0].size() && board[i][j+1] == word[index] && path.find(pair<int, int>{i, j+1}) == path.end() && dfs(board, word, i, j+1, path, index + 1))
            return true;
